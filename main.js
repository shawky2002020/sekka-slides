(function () {
  ...
  function fit() {
    ...
    const scale = Math.min(
      (innerWidth - marginW) / 1440,
      (innerHeight - marginH) / 810,
    );
    document.documentElement.style.setProperty(
      "--s",
      Math.max(0.2, scale).toFixed(4),
    );
    snap(false);
  }
  function update() {
  ...
  function go(i, smooth = true) {
    ...
    update();
    locked = true;
    if (!smooth) deck.classList.add("no-smooth");
    else deck.classList.remove("no-smooth");
    ...
    setTimeout(
      () => {
        locked = false;
        deck.classList.remove("no-smooth");
      },
      smooth ? 640 : 80,
    );
  }
  function nearest() {
    return Math.round(deck.scrollTop / Math.max(1, innerHeight));
  }
  ...
  deck.addEventListener(
    ...
    (e) => {
      ...
      if (Math.abs(dy) > 34 && !locked) go(current + (dy > 0 ? 1 : -1));
    },
    { passive: true },
  );
  ...
  deck.addEventListener("scroll", () => {
    ...
    window.__snapTimer = setTimeout(() => snap(true), 140);
  });
